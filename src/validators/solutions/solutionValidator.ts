export const validateSolutionData = (
  data: {
    name?: string;
    slug?: string;
  ...
  if (!isUpdate) {
    if (!data.name) errors.push("Solution name is required");
  }

  if (data.name !== undefined) {
    if (data.name.trim().length === 0) {
      errors.push("Solution name cannot be empty");
    }
    if (data.name.length > 255) {
      errors.push("Solution name is too long (max 255 characters)");
    }
  }
